    ...
    for epoch in itertools.count(start=1):
        ...
        for start_index in range(0, len(train_parse), args.batch_size):
            ...
            for tree in train_parse[start_index:start_index + args.batch_size]:
                ...
                # current_processed += 1
            if args.parser_type == "top-down":
                _, batch_losses = parser.parse(sentences, gold_trees, args.explore)
            total_processed += len(gold_trees)
            current_processed += len(gold_trees)
...
def run_eval(parser, test_treebank):
    sentences = []
    ...
    parser.eval()
    predicted, _ = parser.parse(sentences)

    test_predicted = []
    for p in predicted:
        test_predicted.append(p.convert())
